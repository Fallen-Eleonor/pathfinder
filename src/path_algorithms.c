...
static void find(t_part_path path[], int **graph, char *current_name, t_list *names_vertices, int current_distance, int n, int a, int total) {
    for (int i = 0; i < n; i++) {
        if (graph[a][i] != 0 && (total + graph[a][i]) == current_distance && mx_strcmp(mx_get_value_from_index(names_vertices, i), current_name) == 0) {
            mx_add_to_path(path, mx_get_value_from_index(names_vertices, i), graph[a][i]);
            mx_print_path(path, current_name, current_distance);
            mx_delete_from_path(path);
            continue;
        }
        if (graph[a][i] != 0 && (total + graph[a][i]) < current_distance) {
            mx_add_to_path(path, mx_get_value_from_index(names_vertices, i), graph[a][i]);
            find(path, graph, current_name, names_vertices, current_distance, n, i, total + graph[a][i]);
        }
    }
    mx_delete_from_path(path);
}
...
static void find_similar_pathway(int **graph, char *current_name, t_list *names_vertices, int current_distance, int n) {
    ...
    mx_init_path(path, n);
    mx_add_to_path(path, mx_get_value_from_index(names_vertices, 0), 0);
