...
static void print_error(int line, t_pathfinder_main data, char *str_line, t_list *nodes) {
    mx_printstr("error: line ");
    mx_printint(line);
    mx_printstr(" is not valid\n");
    free(str_line);
    free(data.file_text);
    mx_delete_list_nodes(nodes);
    exit(-1);
}

static void create_node(t_list **nodes, char *str_line, int line, long int *sum_of_bridges, t_pathfinder_main data) {
    ...
    t_graph_node *new_node = malloc(sizeof(t_graph_node));
    ...
    free(third_element);
    mx_push_back(nodes, (t_graph_node *)new_node);
    mx_error_sum_of_bridges_lengths((*sum_of_bridges) += distance, data, str_line);
}
...
t_list *mx_get_names_vertices(t_list *nodes, int *count) {
    ...
    mx_delete_list(all_names_vertices);
...
t_list *mx_get_nodes(t_pathfinder_main data) {
    ...
    for (int i = 0; data.file_text[i] != '\0'; i++) {
        ...
        line_len != -1 ? free(correct_line) : free(incorrect_line);
        line++;
        i += line_len;
        if (line_len == -1) {
            break;
